...
class LeadStatusAdapter(var activity: Activity, var useractivitylist: ArrayList<LeadModel.Counts>, var onClickEmail1: OnEmailClick) :
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        statementHandler.status_count.text = item.count?.toInt().toString()
        statementHandler.status_type.text = item.displayName.toString()

        // Set background color based on ID
        when (item.id) {
            -1 -> statementHandler.relll_main.setBackgroundColor(Color.parseColor("#182D69"))
            0 -> statementHandler.relll_main.setBackgroundColor(Color.parseColor("#47AA4E"))
            5 -> statementHandler.relll_main.setBackgroundColor(Color.parseColor("#FC4F38"))
            6 -> statementHandler.relll_main.setBackgroundColor(Color.parseColor("#FDA73E"))
            7 -> statementHandler.relll_main.setBackgroundColor(Color.parseColor("#1793D1"))
        ...
        statementHandler.relll_main.setOnClickListener {
            onClickEmail1.onClickEmail1(item.id)
        }
