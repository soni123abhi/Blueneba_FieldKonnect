...
class ExpenselistAdapter(var activity: Activity, var userexpenselist: ArrayList<UserExpenseListModel.Data>, var onClickEmail_expense: OnEmailClick) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.report_expenselistlayout, parent, false)
...
//            flag = "Create"
            onClickEmail_expense.onClickEmail_expense(userexpenselist[position].id.toString(),userexpenselist.get(position).status.toString()
                ,userexpenselist.get(position).date.toString())
        }
...
//        var cardbtn_view: CardView = itemView.findViewById(R.id.cardbtn_view)
    ...
    interface OnEmailClick {
        fun onClickEmail_expense(id: String, status: String, date: String)
    }
}