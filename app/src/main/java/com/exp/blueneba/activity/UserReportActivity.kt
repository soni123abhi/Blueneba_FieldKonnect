package com.exp.blueneba.activity
...
import com.exp.blueneba.adapter.UserActivityAdapter
import com.exp.blueneba.adapter.UserActivityDetailAdapter
import com.exp.blueneba.connection.APIResultLitener
import com.exp.blueneba.connection.ApiClient
import com.exp.blueneba.fragment.add
...
import com.exp.blueneba.model.UserActivityDetailModel
...
class UserReportActivity : AppCompatActivity(),View.OnClickListener,UserActivityAdapter.OnEmailClick {
    ...
    private fun initViews() {
        ...
        rec_user_main = findViewById(R.id.rec_user_main)
        rec_user_main2 = findViewById(R.id.rec_user_main2)
        ...
        cardBack.setOnClickListener {
            if (flag.equals("")){
                startActivity(Intent(this@UserReportActivity, ReportActivity::class.java))
            }else{
                startActivity(Intent(this@UserReportActivity, UserReportActivity::class.java))
            }
        }
    ...
    private fun setuprecycleruserlist() {
        var mLayoutManager = LinearLayoutManager(this@UserReportActivity)
        recyclerView_useractivity.layoutManager = mLayoutManager
        val useractivityAdapter = UserActivityAdapter(this@UserReportActivity, useractivitylist,this)
    ...
    private fun getuseractivity(page: Int, start_date: String, end_date: String, searchbranch: ArrayList<String>, selecteduser_id: String) {
        ...
        if (!Utilities.isOnline(this@UserReportActivity)) {
            isLoading = false
            return
        }
        var dialog = DialogClass.progressDialog(this@UserReportActivity)
        ...
        ApiClient.getUserActivity(
            StaticSharedpreference.getInfo(Constant.ACCESS_TOKEN, this@UserReportActivity).toString(),
            ...
            object : APIResultLitener<UserActivityListModel> {
                ...
                override fun onAPIResult(response: Response<UserActivityListModel>?, errorMessage: String?) {
                    ...
                    if (response != null && errorMessage == null) {
                        ...
                        Toast.makeText(this@UserReportActivity, resources.getString(R.string.poor_connection), Toast.LENGTH_LONG).show()
                    }
                }
            })
    }
        ...
        when (p0!!.id) {
            ...
            R.id.cardSearch -> {
                ...
                if (start_date.equals("")){
                    Toast.makeText(this@UserReportActivity,"Please Select Start Date",Toast.LENGTH_LONG).show()
                }else if (end_date.equals("")){
                    Toast.makeText(this@UserReportActivity,"Please Select End Date",Toast.LENGTH_LONG).show()
                }else{
                    getuseractivity(page, start_date, end_date, searchbranch, selecteduser_id)
                }
            }
    ...
    private fun setuprecycler(useractivitydetail: ArrayList<UserActivityDetailModel>) {
        ...
        val useractivity = UserActivityDetailAdapter(this@UserReportActivity, useractivitydetail)
